...
pub fn WorkloadType(comptime AccountingStateMachine: type) type {
    ...
    const Action = enum(u8) {
        ...
        // ArcherDB GeoEvent operations (F1.3.5)
    ...
    return struct {
        ...
        // GeoEvent state tracking (F1.3.5)
        /// The index of the next geo_event to send.
        geo_events_sent: usize = 0,
        /// All geo_events below this index have been delivered.
        geo_events_delivered: usize = 0,
            ...
            switch (action) {
                inline else => |action_comptime| {
                    ...
                    const count = switch (action_comptime) {
                        ...
                        // ArcherDB GeoEvent operations (F1.3.5)
                        .insert_events => self.build_insert_events(
                            client_index,
                            batchable,
                        ),
                        .query_uuid => self.build_query_uuid_filter(
                            client_index,
                            batchable,
                        ),
                        .query_latest => self.build_query_latest_filter(
                            client_index,
                            batchable,
                        ),
                    };
        ...
        pub fn on_reply_batch(
            ...
            switch (operation) {
                ...
                // ArcherDB GeoEvent operations (F1.3.5)
                .insert_events => self.on_insert_events(
                    client_index,
                    timestamp,
                    stdx.bytes_as_slice(.exact, tb.GeoEvent, request_body),
                    stdx.bytes_as_slice(.exact, tb.InsertGeoEventsResult, reply_body),
                ),
                .query_uuid => self.on_query_uuid(
                    timestamp,
                    stdx.bytes_as_slice(.exact, tb.QueryUuidFilter, request_body),
                    stdx.bytes_as_slice(.exact, tb.GeoEvent, reply_body),
                ),
                .query_latest => self.on_query_latest(
                    timestamp,
                    stdx.bytes_as_slice(.exact, tb.QueryLatestFilter, request_body),
                    stdx.bytes_as_slice(.exact, tb.GeoEvent, reply_body),
                ),

                // ArcherDB operations not yet generated by workload (F1.3.5 partial)
                .upsert_events,
                .delete_entities,
                .query_radius,
                .query_polygon,
        ...
        fn batch(
            ...
            const batch_min = switch (action) {
                ...
                .get_change_events,
                => 1,
                // GeoEvent operations (F1.3.5)
                .insert_events => self.options.geo_events_batch_size_min,
                .query_uuid,
                .query_latest,
                => 1,
            };
            const batch_span = switch (action) {
                ...
                => 0,
                // GeoEvent operations (F1.3.5)
                .insert_events => self.options.geo_events_batch_size_span,
                .query_uuid,
                .query_latest,
                => 0,
            };
        ...
        /// Build GeoEvent insert request with random valid coordinates
        fn build_insert_events(
            ...
            for (events) |*event| {
                const event_index = self.geo_events_sent;
                const event_id = self.geo_event_index_to_id(event_index);
                ...
                const lon_nano: i64 = @as(i64, @intCast(self.prng.int_inclusive(u64, lon_range))) +
                    tb.GeoEvent.lon_nano_min;

                // Generate a simple S2 cell ID (just use lat/lon hash for now)
                const s2_cell_id: u64 = @bitCast(@as(i64, @truncate(
                    @as(i128, lat_nano) *% @as(i128, lon_nano),
                )));

                event.* = std.mem.zeroInit(tb.GeoEvent, .{
                    .id = tb.GeoEvent.pack_id(s2_cell_id, 0), // timestamp set by SM
                    .entity_id = event_id,
                    ...
                    .heading_cdeg = self.prng.int_inclusive(u16, tb.GeoEvent.heading_max),
                    .flags = .{},
                });

                // Maybe generate invalid coordinates
                if (self.prng.chance(self.options.create_geo_event_invalid_probability)) {
                    // Set invalid latitude (out of range)
                    event.lat_nano = tb.GeoEvent.lat_nano_max + 1;
                }

                self.geo_events_sent += 1;
            }
            return events.len;
        }

        /// Build query_uuid filter
        fn build_query_uuid_filter(
            self: *const Workload,
            client_index: usize,
            body: []tb.QueryUuidFilter,
        ) usize {
            _ = client_index;
            assert(body.len == 1);
            ...
            // If we've sent events, query for one of them
            if (self.geo_events_sent > 0) {
                const event_index = self.prng.int_inclusive(usize, self.geo_events_sent - 1);
                const entity_id = self.geo_event_index_to_id(event_index);
                filter.* = std.mem.zeroInit(tb.QueryUuidFilter, .{
                    .entity_id = entity_id,
                    .limit = self.prng.int_inclusive(u32, 100),
                });
            } else {
                // No events yet, query for random UUID
                filter.* = std.mem.zeroInit(tb.QueryUuidFilter, .{
        ...
        /// Build query_latest filter
        fn build_query_latest_filter(
            self: *const Workload,
            client_index: usize,
            body: []tb.QueryLatestFilter,
        ) usize {
            _ = client_index;
            assert(body.len == 1);

            const filter = &body[0];
            filter.* = std.mem.zeroInit(tb.QueryLatestFilter, .{
                .limit = self.prng.int_inclusive(u32, 100),
                .group_id = if (self.prng.boolean())
                    self.prng.int_inclusive(u64, 100) // Filter by group
                else
                    0, // All groups
            });
            return 1;
        ...
        /// Handle insert_events response
        fn on_insert_events(
            self: *Workload,
            client_index: usize,
            timestamp: u64,
            events: []const tb.GeoEvent,
            results_sparse: []const tb.InsertGeoEventsResult,
        ) void {
            _ = client_index;
            _ = timestamp;

            // For each event without an error, it was successfully inserted
            var error_index: usize = 0;
            for (events, 0..) |_, i| {
                const has_error = error_index < results_sparse.len and
                    results_sparse[error_index].index == i;
                if (!has_error) {
                    // Event was successfully inserted
                    self.geo_events_delivered += 1;
                } else {
                    error_index += 1;
                }
            }
        ...
        /// Handle query_uuid response
        fn on_query_uuid(
            self: *Workload,
            timestamp: u64,
            body: []const tb.QueryUuidFilter,
            results: []const tb.GeoEvent,
        ) void {
            _ = self;
            _ = timestamp;
            assert(body.len == 1);

            // Validate results are for the requested entity
            const filter = &body[0];
            for (results) |*event| {
                assert(event.entity_id == filter.entity_id);
                // Validate coordinate bounds
                assert(event.lat_nano >= tb.GeoEvent.lat_nano_min);
                assert(event.lat_nano <= tb.GeoEvent.lat_nano_max);
                assert(event.lon_nano >= tb.GeoEvent.lon_nano_min);
                assert(event.lon_nano <= tb.GeoEvent.lon_nano_max);
            }
        }

        /// Handle query_latest response
        fn on_query_latest(
            self: *Workload,
            timestamp: u64,
            body: []const tb.QueryLatestFilter,
            results: []const tb.GeoEvent,
        ) void {
            _ = self;
            _ = timestamp;
            ...
            const filter = &body[0];
            // Validate results respect limit
            assert(results.len <= filter.limit);

            // Validate results are in descending timestamp order
            var prev_timestamp: u64 = std.math.maxInt(u64);
            for (results) |*event| {
                assert(event.timestamp <= prev_timestamp);
                prev_timestamp = event.timestamp;

                // If group_id filter was set, validate it
                if (filter.group_id != 0) {
                    assert(event.group_id == filter.group_id);
                }
        ...
        /// Convert geo_event index to deterministic ID
        fn geo_event_index_to_id(self: *const Workload, index: usize) u128 {
            // +1 so that index=0 is encoded as a valid id.
...
fn OptionsType(
    ...
    return struct {
        ...
        // GeoEvent options (F1.3.5)
        geo_event_id_permutation: IdPermutation,
        geo_events_batch_size_min: usize,
        geo_events_batch_size_span: usize, // inclusive
        /// Probability of creating an invalid GeoEvent (bad coordinates, etc.)
        create_geo_event_invalid_probability: Ratio,
        ...
        pub fn generate(prng: *stdx.PRNG, options: struct {
            ...
            return .{
                ...
                .operations = .{
                    ...
                    // GeoEvent operations (F1.3.5)
                    .insert_events = prng.range_inclusive(u64, 1, 50),
                    .query_uuid = prng.range_inclusive(u64, 1, 20),
                    .query_latest = prng.range_inclusive(u64, 1, 20),
                },
                ...
                // GeoEvent options (F1.3.5)
                .geo_event_id_permutation = IdPermutation.generate(prng),
                .geo_events_batch_size_min = 0,
                .geo_events_batch_size_span = prng.range_inclusive(
                    usize,
                    1,
                    Operation.insert_events.event_max(options.batch_size_limit),
                ),
                .create_geo_event_invalid_probability = ratio(1, 100),
            };
        }
    };
}
