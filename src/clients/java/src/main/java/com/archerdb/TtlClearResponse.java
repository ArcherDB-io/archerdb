...
public final class TtlClearResponse extends Batch {
     ...
     */
    public TtlClearResponse(final int capacity) {
        super(capacity, Struct.SIZE);
    }

    TtlClearResponse(final ByteBuffer buffer) {
     ...
     */
    byte[] getReserved() {
        return getArray(at(Struct.Reserved), 36);
    }
    ...
    void setReserved(byte[] reserved) {
        if (reserved == null)
            reserved = new byte[36];
        if (reserved.length != 36)
            throw new IllegalArgumentException("Reserved must be 36 bytes long");
