...
using static ArcherDB.AssertionException;
using static ArcherDB.Native;
...
internal sealed class NativeClient : IDisposable
{
    ...
    /// </summary>
    private readonly TBClient[] arch_client;
    ...
    private NativeClient(TBClient[] arch_client)
    {
        AssertTrue(arch_client.Length == 1);
    ...
    public static NativeClient Init(UInt128 clusterID, string[] addresses)
    {
        unsafe
        {
            return CallInit(arch_client_init, clusterID, addresses);
        }
    }
    ...
    public static NativeClient InitEcho(UInt128 clusterID, string[] addresses)
    {
        unsafe
        {
            return CallInit(arch_client_init_echo, clusterID, addresses);
        }
    }
    ...
    private static NativeClient CallInit(InitFunction initFunction, UInt128Extensions.UnsafeU128 clusterID, string[] addresses)
    {
        ...
        unsafe
        {
            ...
            var arch_client = GC.AllocateUninitializedArray<TBClient>(1, pinned: true);
            fixed (TBClient* client = &arch_client[0])
    ...
    public unsafe void Submit(TBPacket* packet)
    {
        unsafe
        {
            fixed (TBClient* client = &arch_client[0])
            {
                var status = arch_client_submit(client, packet);
