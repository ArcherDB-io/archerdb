...
from .lib import arch_assert, c_uint128

logger = logging.getLogger("archerdb")
...
class Client:
    ...
    def _acquire_packet(
        self, operation: bindings.Operation, operations: Any, c_event_type: Any, c_result_type: Any
    ) -> InflightPacket:
        packet = bindings.CPacket()
        packet.next = None
    ...
    def _c_on_completion(
        completion_ctx: int, packet: Any, timestamp: int, bytes_ptr: Any, len_: int
    ) -> None:
        """
        ...
        if packet[0].status != bindings.PacketStatus.OK.value:
            ...
            if inflight_packet.on_completion is None:
                # Can't use arch_assert here, as mypy complains later that it might be None.
        ...
        arch_assert(len_ % ctypes.sizeof(c_result_type) == 0)
        ...
        # dataclass.
        results_slice = ctypes.cast(bytes_ptr, ctypes.POINTER(c_result_type))[
...
class ClientSync(Client, bindings.StateMachineMixin):
    ...
    def _submit(
        ...
        client_state = bindings.arch_client_submit(
            ctypes.byref(self._client), ctypes.byref(inflight_packet.packet)
        )
    ...
    def close(self) -> None:
        arch_assert(self._client is not None)
        bindings.arch_client_deinit(ctypes.byref(self._client))

        arch_assert(len(self._inflight_packets) == 0)
...
class ClientAsync(Client, bindings.AsyncStateMachineMixin):
    def _on_completion(self, inflight_packet: InflightPacket) -> None:
        ...
        inflight_packet.on_completion_context.loop.call_soon_threadsafe(
            self._trigger_event, inflight_packet
        )
    ...
    async def _submit(
        ...
        inflight_packet.on_completion_context = CompletionContextAsync(
            loop=asyncio.get_event_loop(), event=asyncio.Event()
        )
    ...
    async def close(self) -> None:
        ...
        # arch_client_deinit internally clears any inflight requests, and calls their callbacks, so
...
arch_assert(
    bindings.arch_client_register_log_callback(log_handler, True)
...
def configure_logging(
    *,
    ...
    arch_assert(
        bindings.arch_client_register_log_callback(None, debug)
        == bindings.RegisterLogCallbackStatus.SUCCESS
    )

    arch_assert(
        bindings.arch_client_register_log_callback(handler, debug)
        == bindings.RegisterLogCallbackStatus.SUCCESS
    )
        bindings.RegisterLogCallbackStatus.SUCCESS)
