...
pub fn tests(shell: *Shell, gpa: std.mem.Allocator) !void {
    ...
    // Although we have compiled the ArcherDB client library, we still need `cgo` to link it with
    // our resulting Go binary. Strictly speaking, `CC` is controlled by the users of ArcherDB,
...
pub fn validate_release(shell: *Shell, gpa: std.mem.Allocator, options: struct {
    ...
    try shell.exec("go mod init archtest", .{});
    try shell.exec("go get github.com/archerdb/archerdb-go@v{version}", .{
...
pub fn release_published_latest(shell: *Shell) ![]const u8 {
    // Example output:
    //  github.com/archerdb/archerdb-go v0.9.149 v0.13.56 v0.13.57
    const output = try shell.exec_stdout(
        "go list -m -versions github.com/archerdb/archerdb-go",
        .{},
    );
